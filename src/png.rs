...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if let Some(index_to_remove) = self.chunks.iter().position(|c| *c.chunk_type() == chunk_type) {
            ...
            Err(anyhow!("did not find any chunks of specified type to remove"))
        }
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        ...
        if let Some(chunk_index) = self.chunks.iter().position(|c| *c.chunk_type() == chunk_type) {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        if value.len() < 8 {
            bail!("too little data to construct a Png")
        }
        if value[0..8] != Png::STANDARD_HEADER {
            bail!("failed to construct Png due to header data mismatch")
        }

        // size of chunk w/o any data
        const BASE_CHUNK_SIZE: usize = 12;  // so total chunk size will be 12 + len of data
        let mut byte_index: usize = 8;  // start at the end of where the header was
        let mut chunk_vec: Vec<Chunk> = Vec::new();
        while byte_index < value.len() {
            let next_chunk = Chunk::try_from(&value[byte_index..]).context("attempt to construct chunks for Png")?;
            let bytes_used: usize = next_chunk.length().try_into()?;
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let header_string: String = Png::STANDARD_HEADER.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(", ");
        let chunks_string = self.chunks.iter().map(|chunk| chunk.to_string()).collect::<Vec<String>>().join(", ");
